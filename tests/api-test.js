...
class APITester {
  ...
  async authenticateWebSocket() {
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('auth-error', (error) => {
        reject(new Error(`WebSocket auth failed: ${error.error}`));
      });
      setTimeout(() => {
  ...
  async runAllTests() {
    ...
    await this.test('Get Queue Stats', async () => {
      ...
      if (!response.success || typeof response.data.movies === 'undefined') {
        throw new Error('Queue stats failed');
      }
      this.log(` Queue stats - Movies: ${response.data.movies}, Gaming: ${response.data.gaming}, Music: ${response.data.music}, Study: ${response.data.study}, Active: ${response.data.activeRooms}`, 'info');
    });
    ...
    await this.test('WebSocket P2P Queue', async () => {
      return new Promise((resolve) => {
        this.socket.emit('find-match', { category: 'gaming' });        
    ...
    await this.test('WebSocket Leave Queue', async () => {
      return new Promise((resolve) => {
        ...
        this.socket.on('matching-cancelled', (data) => {
          if (data.success) {
            resolve();
          }
        });
  ...
  printResults() {
    ...
    this.log(' Features tested: PostgreSQL, P2P matching, dynamic reconnection', 'info');
